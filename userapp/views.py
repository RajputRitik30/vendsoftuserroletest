...
def dashboard(request):
    try:
        ...
        return Response({'success':0,'message':f'Error {e}'})
...
#                 logging.info(f'payment Data: {str(storeName[0].qr_store_name)}')
#                 if len(storeName) != 0:
#                     payload = json.dumps({
...
def paymentcallback(request):
    if request.method == 'POST':
        try:
            encoded_string = request.data['response']
            logger.info(f'Payment Data (encoded): {encoded_string}')
            
            # Decode the base64 encoded JWT token
            jwt_token = base64.urlsafe_b64decode(encoded_string + '=' * (4 - len(encoded_string) % 4))
            logger.info(f'Payment Data (decoded JWT): {jwt_token}')
            
            # Decode JWT token to JSON string
            json_string = jwt_token.decode('utf-8')
            json_data = json.loads(json_string)
            logger.info(f'Payment Data (JSON): {json_data}')
            ...
            if json_data['code'] == 'PAYMENT_SUCCESS':
                ...
                if store_name_record.exists():
                    ...
                    # Publish to MQTT
                    paho.mqtt.publish.single(TOPIC_M_ORDER, payload, hostname=BROKER, port=PORT, auth={'username': MQTT_USERNAME, 'password': MQTT_PASSWORD})
                    logger.info('Payment success and MQTT message published successfully')
                else:
                    logger.warning(f'Store ID {json_data["data"]["storeId"]} not found in the database')
                    return JsonResponse({'success': 0, 'message': 'Store ID not found', 'result': ''})
            else:
                logger.warning(f'Payment failed with code: {json_data["code"]}')
            
            return JsonResponse({'success': 1, 'message': 'Data Found', 'result': json_data})
        
        except Exception as e:
            logger.error(f'Error processing payment: {e}', exc_info=True)
            return JsonResponse({'success': 0, 'message': 'An error occurred', 'result': ''})
