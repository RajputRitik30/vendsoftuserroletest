...
class MachineMasterViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        try:
            ...
            logger.info("MachineMaster list retrieved successfully")
            return Response({'success': 1, 'message': 'Manage Membership List', 'result': serializer.data})
        except Exception as e:
            logger.error(f"Error retrieving MachineMaster list: {str(e)}")
    ...
    def retrieve(self, request, pk, *args, **kwargs):
        try:
            ...
            logger.info(f"MachineMaster retrieved successfully for ID {pk}")
            return Response({'success': 1, 'message': 'Manage Membership', 'result': serializer.data})
        except MachineMaster.DoesNotExist:
            logger.warning(f"MachineMaster not found for ID {pk}")
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            serializer.save()
            logger.info("MachineMaster created successfully")
            return Response({'success': 1, 'message': 'Data Created', 'result': serializer.data})
        except ValidationError as ve:
            logger.error(f"Validation error creating MachineMaster: {ve.detail}")
    ...
    def update(self, request, pk, *args, **kwargs):
        try:
            ...
            if 'rqcode' not in request.data:
                logger.warning(f"rqcode is required for updating MachineMaster with ID {pk}")
            ...
            serializer.is_valid(raise_exception=True)
            serializer.save()
            logger.info(f"MachineMaster updated successfully for ID {pk}")
    ...
    def destroy(self, request, pk, *args, **kwargs):
        try:
            ...
            if machine.tenant != request.tenant:
                logger.warning(f"Permission denied to delete MachineMaster with ID {pk}")
                return Response({'success': 0, 'message': 'You do not have permission to delete this machine'},
                                status=status.HTTP_403_FORBIDDEN)

            machine.delete()
            logger.info(f"MachineMaster deleted successfully for ID {pk}")
...
def create_mapping_user(request):
    try:
        if request.method == 'POST':
            logger.info("POST request received for creating a user-machine mapping")
            ...
            if not existing_mapping.exists():
                ...
                logger.info(f"Created new mapping for user {user_email} and machine {machine_id}")
                return Response({'success': 1, 'result': serializer.data}, status=status.HTTP_201_CREATED)
            else:
                logger.warning(f"Mapping already exists for user {user_email} and machine {machine_id}")
                return Response({'success': 0, 'result': 'Mapping already exists'}, status=status.HTTP_400_BAD_REQUEST)

        elif request.method == 'GET':
            logger.info("GET request received for listing user-machine mappings")
            ...
            for user in users:
                for machine in machines:
                    ...
                    if not existing_mapping.exists():
                        ...
                        logger.info(f"Created new mapping for user {user.email} and machine {machine.machine_id}")
                    else:
                        serializer = MachineUserMappingSerializer(existing_mapping.first())
                        mappings_data.append(serializer.data)
                        logger.info(f"Existing mapping found for user {user.email} and machine {machine.machine_id}")

            return Response({'success': 1, 'result': mappings_data}, status=status.HTTP_201_CREATED)

    except User.DoesNotExist:
        logger.error(f"User with email {request.data.get('user_email')} not found")
        return Response({'success': 0, 'result': 'User not found'}, status=status.HTTP_404_NOT_FOUND)
    except MachineMaster.DoesNotExist:
        logger.error(f"Machine with ID {request.data.get('machine_id')} not found")
...
def get_machine_user_map_by_customer(request):
    if request.method == 'GET':
        try:
            logger.info(f"GET request received for machine-user mapping by customer {request.user.id}")

            machine_user_mappings = MachineUserMapping.objects.filter(assigned_customer=request.user.id, assigned_user__isnull=False).order_by('-id')
            
            if not machine_user_mappings.exists():
                logger.warning(f"No machine-user mappings found for customer {request.user.id}")
                return Response({'success': 0, 'message': 'No Data Found'}, status=status.HTTP_404_NOT_FOUND)

            for machine_id in machine_user_mappings:
                logger.debug(f"Found mapping: {machine_id}")

            serializer = MachineUserMapping1Serializer(machine_user_mappings, many=True)
            logger.info(f"Successfully retrieved mappings for customer {request.user.id}")

            return Response({'success': 1, 'message': 'Data Found', 'result': serializer.data})
        except Exception as e:
            logger.error(f"Error processing request for customer {request.user.id}: {e}")
...
def get_products(request):
    try:
        logger.info("GET request received for fetching products.")

        products = Product.objects.all()
        if not products.exists():
            logger.warning("No products found.")
            return Response({'success': 0, 'message': 'No Products Found'}, status=status.HTTP_404_NOT_FOUND)
        ...
        logger.info(f"{len(serialized_products)} products retrieved successfully.")
...
def get_modelcapacity(request):
    if request.method == 'GET':
        try:
            logger.info("GET request received for model capacity.")
            
            modelcapacity = ModelCapacity.objects.all()
            if not modelcapacity.exists():
                logger.warning("No model capacity found.")
                return Response({'success': 0, 'message': 'No Model Capacity Found'}, status=status.HTTP_404_NOT_FOUND)
            
            serialized_data = [{'id': mc.id, 'name': mc.name, 'model_no': mc.model_no} for mc in modelcapacity]
            logger.info("Model capacity retrieved successfully.")
            return Response({'success': 1, 'message': 'Model capacity retrieved successfully', 'result': serialized_data})
        except Exception as e:
            logger.error(f"Error retrieving model capacity: {str(e)}")
            return Response({'success': 0, 'message': f'Error: {str(e)}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    
    elif request.method == 'POST':
        try:
            logger.info("POST request received for creating model capacity.")
            ...
            logger.info("Model capacity created successfully.")
            ...
            logger.error(f"Error creating model capacity: {str(e)}")
            return Response({'success': 0, 'message': f'Error: {str(e)}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def read_machine_user_mapping_report_pdf(request):
    try:
        logger.info("Received request to generate machine user mapping report PDF.")
        ...
        if not date_from_parsed or not date_to_parsed:
            logger.warning("Invalid date range provided.")
            return Response({"message": "Please provide both 'from' and 'to' date parameters in YYYY-MM-DD format."}, status=400)

        logger.info(f"Filtering data from {date_from_parsed} to {date_to_parsed}.")
        ...
        # Check if serialized_data is empty
        if not serialized_data:
            logger.info("No data found for the provided date range.")
            return Response({"message": "No data found for the provided date range."}, status=404)

        logger.info("Data found, proceeding to generate PDF.")
        ...
        logger.info(f"PDF generated successfully at {pdf_file_path}.")

        with open(pdf_file_path, 'rb') as pdf_file:
            response.write(pdf_file.read())

        logger.info("PDF returned in response.")
        return response
    except Exception as e:
        logger.error(f"Error generating machine user mapping report PDF: {str(e)}")
        return Response({'success': 0, 'message': f'Error: {str(e)}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def get_assigned_machine_by_user(request):
    try:
        logger.info("Received request to get assigned machine by user.")

        # Retrieve the logged-in user
        user = request.user
        logger.info(f"Retrieved logged-in user: {user.id} - {user.email}")
        ...
        logger.info(f"Retrieved machine assigned to user: {machine_assigned.machine_id}")
        ...
        logger.warning(f"No machine assigned to user: {user.id} - {user.email}")
        return Response({'message': 'No machine assigned to this user'}, status=404)
    except Exception as e:
        logger.error(f"Error retrieving assigned machine for user {user.id} - {user.email}: {str(e)}")
...
def get_mstatus_for_assigned_user(request):
    if request.method == 'GET':
        try:
            logger.info("Received request to get machine status for assigned user.")
            
            # Retrieve machine-user mappings for the logged-in user
            user = request.user
            map_objs = MachineUserMapping.objects.filter(assigned_user=user.id).order_by('-id')
            logger.info(f"Retrieved mappings for user {user.id}.")

            # Get all machine IDs from the retrieved mappings
            machine_ids = [map_obj.machine.machine_id for map_obj in map_objs]
            logger.info(f"Machine IDs for user {user.id}: {machine_ids}")
            ...
            with connection.cursor() as cursor:
                ...
                logger.info(f"Fetched MQTT data since {threshold_time}.")
            ...
            logger.info("Calculated online/offline durations for machines.")
            ...
            logger.info("Fetched latest stock data and prepared the response.")

            return Response({'success': 1, 'message': 'Data Found', 'result': stocks})
        except MachineUserMapping.DoesNotExist:
            logger.warning(f"No machines assigned to user {request.user.id}.")
            return Response({'message': 'No machines assigned to this user'}, status=404)
        except Exception as e:
            logger.error(f"Error processing request for user {request.user.id}: {str(e)}")
            return Response({'message': f'Error: {str(e)}'}, status=500)
...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            logger.error(f'Error {e}')
            return Response({'success': 0, 'message': 'Not Created'})
...
def site_settings_api_for_logo(request):
    try:
        ...
        logger.error(f'Error {e}')
        return Response({'success':0,'message':'Error {e}'})
...
def get_product_list(request):
    try:
        ...
        logger.error(f'Error {e}')
        return Response({'succes':0,'message':str(e)})
...
class ModelCapacityViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            logger.error(f'Error {e}')
            return Response({'success': 0, 'message': 'Not Created', 'result': str(e)})
